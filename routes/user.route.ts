...
router.post('/users', [body('name').isString().notEmpty().withMessage('name required')], validateRequest,async (req: Request, res: Response) => {
...
router.post('/users/:userId/borrow/:bookId',[param('userId').isNumeric(), param('bookId').isNumeric()],validateRequest, async (req: Request, res: Response) => {
  ...
  const borrowedBook = await borrowedBookRepository.findOne({ where: { book_id: bookId, returned: false } });
  if (borrowedBook) {
      return res.status(400).send('This book is already borrowed');
  }
  
  // Create a new borrowed book entry
  const newBorrowedBook = borrowedBookRepository.create({ book_id: bookId, user_id: userId });
...
router.post('/users/:userId/return/:bookId', [body('score').isNumeric().notEmpty()], validateRequest, async (req: Request, res: Response) => {
  const userId = Number(req.params.userId);
  const bookId = Number(req.params.bookId);
  const { score } = req.body;

  // Find the borrowed book
  const borrowedBook = await borrowedBookRepository.findOne({ where: { user_id: userId, book_id: bookId, returned: false } });
