...
router.post(
  "/books",
  [body("name").notEmpty().isString().withMessage("book name is required")],
  validateRequest,
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    const query = await bookRepository
      ...
      .addSelect(
        "ROUND(COALESCE((SELECT AVG(borrowed_book.score) FROM borrowed_book WHERE borrowed_book.book_id = book.id AND borrowed_book.score != -1), -1), 2)",
        "score",
      )
